...
function canCastAbilityLocalFirst(store, me, myState, slot) {
  ...
  // Single source of truth: the server cooldown.
  // Local prediction is allowed for visuals only, not for the readiness/timer.
...
function applyLocalPassiveEffects(store, me, reason = 'hit', amount = 1) {
  store?.noteLocalPassiveEvent?.(reason, amount, { frameId: me?.frameId || store?.myState?.frameId || '', authorityMs: 1250 });
}

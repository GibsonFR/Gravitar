...
export class StationWindowView {
  ...
  updatePendingUi() {
    ...
    if (this.pendingTextEl) {
      if (pending) this.pendingTextEl.textContent = summary.count > 1 ? `${summary.count} actions station en cours…` : 'Action station envoyée…';
      else if (failed) this.pendingTextEl.textContent = 'Action refusée ou serveur indisponible.';
    }
  }
